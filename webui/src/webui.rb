...
get "/admin/metric_list" do
  protected!
  db_conn = db_connect
  @ping_metrics = db_conn[:ping_metrics].limit(50).order(Sequel.desc(:timestamp))
...
get "/admin/probe_list" do
  ...
  begin
    ...
    @probes_unregistered = db_conn[:probes].where(active: 2)
    @probes_inactive = db_conn[:probes].where(active: 0)
...
get "/matrix" do
  ...
  begin
    ...
    @probes_list = db_conn[:probes].where(active: 1).order(Sequel.desc(:location), Sequel.asc(:site)).all
    ...
    active_probes.each do |probe|
      ...
      latest_dest_probes = db_conn[:ping_metrics]
        .join(subquery, {
...
get "/site_details" do
  ...
  begin
    ...
    @ping_metrics = db_conn[:ping_metrics].where(source_site: @source_site, dest_site: @dest_site).order(Sequel.desc(:timestamp)).limit(5)
    traceroute_metrics = db_conn[:traceroute_metrics].where(source_site: @source_site, dest_site: @dest_site).order(Sequel.desc(:timestamp)).first
